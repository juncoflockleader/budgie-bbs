...
export function ThreadPage({
  ...
  // postId -> reaction state. Counts come from projection reads; live reaction
  // events are hints that schedule a refresh because unordered counters converge.
  ...
  const refreshReactionProjection = useCallback(async () => {
    const postsRes = await api.listPosts(token, thread.id)
    const loadedPosts = postsRes.data
    if (!loadedPosts) return

    const countByPost = new Map(loadedPosts.map(post => [post.id, post.reactionCount]))
    setPosts(prev => prev.map(post => {
      const count = countByPost.get(post.id)
      return count === undefined ? post : { ...post, reactionCount: count }
    }))
    ...
    setReactions(prev => {
      ...
      loadedPosts.forEach(post => {
        next[post.id] = {
          ...
          reacted: prev[post.id]?.reacted ?? false,
        }
      })
      return next
    })
  ...
  const scheduleReactionProjectionRefresh = useCallback((postId: string, currentUserReacted?: boolean) => {
    if (currentUserReacted !== undefined) {
      setReactions(prev => ({
        ...prev,
        [postId]: {
          count: prev[postId]?.count ?? 0,
          reacted: currentUserReacted,
        },
      }))
    }
    ...
    reactionRefreshTimer.current = setTimeout(() => {
      ...
      void refreshReactionProjection()
    }, 150)
  }, [refreshReactionProjection])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Init reaction state from the projection snapshot.
  ...
  const onEvent = useCallback((evt: BudgieEvent) => {
    if (evt.event === 'post.appended') {
      ...
      if (p.thread !== thread.id) return
      scheduleReactionProjectionRefresh(p.postId, p.user === currentUsername ? true : undefined)
    } else if (evt.event === 'post.unreacted') {
      const p = evt.payload as PostUnreactedPayload
      if (p.thread !== thread.id) return
      scheduleReactionProjectionRefresh(p.postId, p.user === currentUsername ? false : undefined)
  ...
  async function submitPost() {
    ...
    try {
      ...
      if (api.isPendingCommandAck(res.data)) {
        setComposeNotice('Reply queued; waiting for writer.')
      }
      const resolved = await api.resolveCommandResult(token, res.data, {
        onStatus: status => {
          if (status.status === 'retrying') {
            setComposeNotice(status.error?.message ? `Reply is retrying: ${status.error.message}` : 'Reply is retrying.')
      ...
      if (resolved.error) {
        setComposeError(resolved.error.message)
        alert(resolved.error.message)
        return
      }
      if (!resolved.data?.id) {
        const message = 'Reply completed without a post id. Refresh the thread before trying again.'
  ...
  async function redactPost(postId: string) {
    ...
    const res = await api.execCommandResolved(token, 'redactPost', { post: postId })
    if (res.error) alert(res.error.message)
  }

  async function restorePost(postId: string) {
    const res = await api.execCommandResolved(token, 'restorePost', { post: postId })
    if (res.error) alert(res.error.message)
  }
  ...
  async function purgePost(postId: string) {
    ...
    const res = await api.execCommandResolved(token, 'purgePost', { post: postId, reason })
    if (res.error) alert(res.error.message)
  }

  async function toggleLock() {
    const res = await api.execCommandResolved(token, 'lockThread', { thread: thread.id, locked: !threadLocked })
    if (res.error) alert(res.error.message)
  }
  ...
  async function renameThreadTitle() {
    ...
    const res = await api.execCommandResolved(token, 'setThreadTitle', { thread: thread.id, title })
    if (res.error) {
      ...
      {canReplyInBoard && (
        composing ? (
          <div className={`compose-box${draftFullScreen ? ' compose-fullscreen' : ''}`}>
            ...
            <div className={draftPreviewOpen ? 'compose-layout' : undefined}>
              <textarea
                ...
                onChange={e => {
                  ...
                  if (composeNotice) setComposeNotice(null)
                }}
            ...
            {composeNotice && <p className="muted">{composeNotice}</p>}
            {canAttach && (
