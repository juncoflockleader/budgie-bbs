  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (api.isPendingCommandAck(res.data)) {
        setNotice('Command queued; waiting for writer.')
      }
      const resolved = await api.resolveCommandResult(token, res.data, {
        onStatus: status => {
          if (status.status === 'retrying') {
            setNotice(status.error?.message ? `Command is retrying: ${status.error.message}` : 'Command is retrying.')
      ...
      if (!resolved.data?.id) {
        setError('Command completed without a thread id. Refresh the board before trying again.')
        return
      }
      removeComposeDraft(draftKey)
      onCreated(resolved.data.id)
    } catch (err) {
