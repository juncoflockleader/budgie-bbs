...
export function AuthorPostsPage({ token, username, onBack, onOpenThread }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    api.listAuthorPosts(token, username, PAGE_SIZE, offset).then(res => {
      if (cancelled) return
      setLoading(false)
      if (res.error) {
  ...
  return (
    <div className="author-posts-page">
      ...
      {loading ? (
        ...
        <p className="muted">No readable posts found.</p>
      ) : (
            ...
            <article key={post.id} className="author-post-row">
              <button className="author-post-target" onClick={() => openPost(post)} disabled={!post.board}>
                ...
                <span className="item-meta muted">
                  {post.boardName || post.board || 'board'} / #{post.createdSeq}
                </span>
              </button>
