...
This status summarizes the current gap against `doc/ui-multilingual-support-design.md` for adding first-class UI localization with English fallback plus Simplified Chinese (`zh-CN`) and Traditional Chinese (`zh-TW`).
...
- Web build currently fails. Running `/opt/homebrew/bin/npm run build` in `web/` reports missing typed i18n keys, mostly under `board.*` and `thread.*`.
...
## Known Web Build Blockers

Missing keys referenced from the Web UI include examples such as:

- `board.across`
- `board.actionMarkRead`
- `board.actionRestoreReadMarker`
- `board.actionShowInUnread`
- `board.actionHideFromUnread`
- `board.actionAddFavorite`
- `board.actionRemoveFavorite`
- `board.favoriteActions.moveFolderUp`
- `board.favoriteActions.moveFolderDown`
- `board.favoriteActions.newFolder`
- `board.categories`
- `board.memberRole.*`
- `board.approvalMode.*`
- `board.settings`
- `board.saveSettings`
- `board.saveRequirements`
- `thread.by`

There are also unused values in `ThreadListPage.tsx` from the partial migration, including `locale`, `plural`, and `memberRoleLabel`.
...
- Re-run `web` build until it passes.
