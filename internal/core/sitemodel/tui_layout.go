...
// TUI. A nil/empty layout renders the built-in default layout.
...
// NormalizeTUIMainMenuLayout validates and canonicalizes a layout. It enforces
// a single menu block, known types/aligns, and size bounds. An empty layout
// returns the default.
...
// StockTUIArtNames returns the sorted list of available stock art keys.
...
// EncodeTUIMainMenuLayout returns the JSON-backed storage value for a layout.
...
// DecodeTUIMainMenuLayout returns the stored layout, falling back to defaults
// when the storage value is blank, invalid, or empty.
