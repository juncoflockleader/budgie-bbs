...
func newTestCore(t *testing.T) (*core.Core, context.CancelFunc) {
	return newTestCoreWithOptions(t)
}

func newTestCoreWithOptions(t *testing.T, options ...core.Option) (*core.Core, context.CancelFunc) {
	t.Helper()
	if dsn := os.Getenv("BUDGIE_TEST_POSTGRES_DSN"); dsn != "" {
		return newTestCorePostgres(t, dsn, options...)
	}
...
func waitForTestEvent(t *testing.T, sub *core.Subscription, kind proto.EventKind) *proto.Event {
	t.Helper()
...
func TestIdempotencyIsPartitionScoped(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	...
	general := c.ExecCmd(context.Background(), admin, proto.CmdCreateThread, generalRaw, cid)
	if general.Err != nil {
		t.Fatalf("general create failed: %s", general.Err.Message)
	}
	life := c.ExecCmd(context.Background(), admin, proto.CmdCreateThread, lifeRaw, cid)
	if life.Err != nil {
		t.Fatalf("life create with same cid in different partition failed: %s", life.Err.Message)
	}
	if general.Result.ID == life.Result.ID {
		t.Fatalf("different partitions returned same result ID %q", general.Result.ID)
	}

	generalThreads, err := c.ListThreads("general", 100, 0)
	if err != nil {
		t.Fatal(err)
	}
	lifeThreads, err := c.ListThreads("life", 100, 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(generalThreads) != 1 || len(lifeThreads) != 1 {
		t.Fatalf("expected one thread in each partition, got general=%d life=%d", len(generalThreads), len(lifeThreads))
	}
}

func TestIdempotencyConflictStaysWithinPartition(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	...
	first := c.ExecCmd(context.Background(), alice, proto.CmdCreateThread, firstRaw, cid)
	if first.Err != nil {
		t.Fatalf("first create failed: %s", first.Err.Message)
	}
	conflict := c.ExecCmd(context.Background(), alice, proto.CmdCreateThread, conflictRaw, cid)
	if conflict.Err == nil {
		t.Fatal("expected same-partition cid reuse with different payload to conflict")
	}
	if conflict.Err.Code != proto.ErrConflict {
		t.Fatalf("conflict code = %s, want %s", conflict.Err.Code, proto.ErrConflict)
	}
}
...
func TestPresenceAccruesTotalOnlineTime(t *testing.T) {
	...
	alice := registerAndGetUser(t, c, "alice", "pw")
	base := time.Date(2026, time.June, 11, 12, 0, 0, 0, time.UTC).UnixMilli()
...
func TestPresenceCoalescesUnchangedOnlinePings(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()

	alice := registerAndGetUser(t, c, "alice", "pw")
	base := time.Date(2026, time.June, 11, 12, 0, 0, 0, time.UTC).UnixMilli()
	readSession := func() (status, mode, boardID, locationLabel string, lastSeen int64) {
		t.Helper()
		if err := c.DB.QueryRow(testRebind(
			`SELECT status, mode, board_id, location_label, last_seen
			   FROM user_presence_sessions
			  WHERE user_id=? AND session_id=?`),
			alice.ID,
			"web",
		).Scan(&status, &mode, &boardID, &locationLabel, &lastSeen); err != nil {
			t.Fatal(err)
		}
		return status, mode, boardID, locationLabel, lastSeen
	}
	readOnlineSeconds := func() int64 {
		t.Helper()
		var seconds int64
		err := c.DB.QueryRow(testRebind(`SELECT total_online_seconds FROM user_activity WHERE user_id=?`), alice.ID).Scan(&seconds)
	...
	_, _, _, _, lastSeen := readSession()
	if lastSeen != base {
		t.Fatalf("initial last_seen = %d, want %d", lastSeen, base)
	}

	if err := projections.SetUserPresence(c.DB, alice.ID, "web", "active", "", "", "", "", "", base+10*1000); err != nil {
		t.Fatal(err)
	}
	_, _, _, _, lastSeen = readSession()
	if lastSeen != base {
		t.Fatalf("unchanged rapid ping last_seen = %d, want unchanged %d", lastSeen, base)
	}
	if got := readOnlineSeconds(); got != 0 {
		t.Fatalf("rapid coalesced ping accrued %d online seconds, want 0", got)
	}

	if err := projections.SetUserPresence(c.DB, alice.ID, "web", "active", "", "", "", "", "", base+35*1000); err != nil {
		t.Fatal(err)
	}
	_, _, _, _, lastSeen = readSession()
	if want := base + 35*1000; lastSeen != want {
		t.Fatalf("post-window ping last_seen = %d, want %d", lastSeen, want)
	}
	if got := readOnlineSeconds(); got != 35 {
		t.Fatalf("post-window ping accrued %d online seconds, want 35", got)
	}

	if err := projections.SetUserPresence(c.DB, alice.ID, "web", "active", "reading", "general", "", "General", "", base+40*1000); err != nil {
		t.Fatal(err)
	}
	status, mode, boardID, locationLabel, lastSeen := readSession()
	if status != "active" || mode != "reading" || boardID != "general" || locationLabel != "General" || lastSeen != base+40*1000 {
		t.Fatalf("changed presence did not persist immediately: status=%q mode=%q board=%q location=%q lastSeen=%d", status, mode, boardID, locationLabel, lastSeen)
	}

	if err := projections.SetUserPresence(c.DB, alice.ID, "web", "offline", "", "", "", "", "", base+45*1000); err != nil {
		t.Fatal(err)
	}
	status, _, _, _, lastSeen = readSession()
	if status != "offline" || lastSeen != base+45*1000 {
		t.Fatalf("offline presence did not persist immediately: status=%q lastSeen=%d", status, lastSeen)
	}
}

func TestGuestPresenceCoalescesUnchangedOnlinePings(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()

	base := time.Date(2026, time.June, 11, 12, 0, 0, 0, time.UTC)
	readSession := func() (status, locationLabel, fromHost string, lastSeen int64) {
		t.Helper()
		if err := c.DB.QueryRow(testRebind(
			`SELECT status, location_label, from_host, last_seen
			  ...
			  WHERE session_id=?`),
			"guest-web",
		).Scan(&status, &locationLabel, &fromHost, &lastSeen); err != nil {
			t.Fatal(err)
		}
		return status, locationLabel, fromHost, lastSeen
	}

	if err := c.SetGuestPresence("guest-web", "active", "lobby", "203.0.113.25", base); err != nil {
		t.Fatal(err)
	}
	_, _, _, lastSeen := readSession()
	if want := base.UnixMilli(); lastSeen != want {
		t.Fatalf("initial guest last_seen = %d, want %d", lastSeen, want)
	}

	if err := c.SetGuestPresence("guest-web", "active", "lobby", "203.0.113.25", base.Add(10*time.Second)); err != nil {
		t.Fatal(err)
	}
	_, _, _, lastSeen = readSession()
	if want := base.UnixMilli(); lastSeen != want {
		t.Fatalf("unchanged rapid guest ping last_seen = %d, want unchanged %d", lastSeen, want)
	}

	if err := c.SetGuestPresence("guest-web", "active", "lobby", "203.0.113.25", base.Add(35*time.Second)); err != nil {
		t.Fatal(err)
	}
	_, _, _, lastSeen = readSession()
	if want := base.Add(35 * time.Second).UnixMilli(); lastSeen != want {
		t.Fatalf("post-window guest ping last_seen = %d, want %d", lastSeen, want)
	}

	if err := c.SetGuestPresence("guest-web", "active", "thread:general", "203.0.113.25", base.Add(40*time.Second)); err != nil {
		t.Fatal(err)
	}
	status, locationLabel, fromHost, lastSeen := readSession()
	if status != "active" || locationLabel != "thread:general" || fromHost != "203.0.113.25" || lastSeen != base.Add(40*time.Second).UnixMilli() {
		t.Fatalf("changed guest presence did not persist immediately: status=%q location=%q host=%q lastSeen=%d", status, locationLabel, fromHost, lastSeen)
	}

	if err := c.SetGuestPresence("guest-web", "offline", "thread:general", "203.0.113.25", base.Add(45*time.Second)); err != nil {
		t.Fatal(err)
	}
	status, _, _, lastSeen = readSession()
	if status != "offline" || lastSeen != base.Add(45*time.Second).UnixMilli() {
		t.Fatalf("offline guest presence did not persist immediately: status=%q lastSeen=%d", status, lastSeen)
	}
	stats, err := c.GetCommunityStats()
	if err != nil {
		t.Fatal(err)
	}
	if stats.TotalGuestLogins != 1 || stats.TotalGuestLogouts != 1 || stats.OnlineGuests != 0 {
		t.Fatalf("guest transition totals changed unexpectedly: %+v", stats)
	}
}

func TestMemoryPresenceStoreBacksCommandPresenceAndOnlineReads(t *testing.T) {
	store := core.NewMemoryPresenceStore()
	c, cancel := newTestCoreWithOptions(t, core.WithPresenceStore(store))
	defer cancel()
	...
	if len(globalOnline) != 1 || globalOnline[0].Name != "bob" || globalOnline[0].Status != "active" || globalOnline[0].Mode != "reading" || globalOnline[0].BoardID != "general" {
		t.Fatalf("expected bob online from memory presence store, got %+v", globalOnline)
	}
	if globalOnline[0].SessionID != "web" || globalOnline[0].LocationLabel != "General" || globalOnline[0].FromHost != "203.0.113.9" {
		t.Fatalf("expected rich memory presence metadata, got %+v", globalOnline[0])
	}
	...
	if len(boardOnline) != 1 || boardOnline[0].Name != "bob" || boardOnline[0].BoardName != "General" {
		t.Fatalf("expected bob in board-scoped memory online list, got %+v", boardOnline)
	}
	...
	if len(notifs) != 1 || notifs[0].Kind != "login" || notifs[0].Actor != "bob" {
		t.Fatalf("expected memory-backed visible presence to satisfy login watch, got %+v", notifs)
	}

	var sessionRows int
	if err := c.DB.QueryRow(testRebind(`SELECT COUNT(*) FROM user_presence_sessions WHERE user_id=?`), bob.ID).Scan(&sessionRows); err != nil {
		t.Fatal(err)
	}
	if sessionRows != 0 {
		t.Fatalf("memory presence store wrote %d SQL session rows, want 0", sessionRows)
	}
	var summaryRows int
	if err := c.DB.QueryRow(testRebind(`SELECT COUNT(*) FROM user_presence WHERE user_id=?`), bob.ID).Scan(&summaryRows); err != nil {
		t.Fatal(err)
	}
	if summaryRows != 0 {
		t.Fatalf("memory presence store wrote %d SQL summary rows, want 0", summaryRows)
	}
	...
	if len(globalOnline) != 1 || globalOnline[0].Status != "active" || globalOnline[0].Mode != "reading" {
		t.Fatalf("typing should not overwrite memory-backed roster, got %+v", globalOnline)
	}
	...
	if len(globalOnline) != 0 {
		t.Fatalf("invisible memory-backed presence should be hidden, got %+v", globalOnline)
	}
}
...
func TestTypingPresenceIsBrokerOnly(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	...
	exec(t, c, bob, proto.CmdSetPresence, proto.SetPresencePayload{
		...
		Mode:      "reading",
		Board:     "general",
		Location:  "General board",
	})
	sub := c.Subscribe([]string{"presence:global"})
	defer c.Unsubscribe(sub)
	...
	select {
	case evt := <-sub.Ch:
		if evt.Kind != proto.EvtPresenceUpdate {
			t.Fatalf("typing event kind = %s, want presence.update", evt.Kind)
		}
		payload, ok := evt.Payload.(*proto.PresenceUpdatePayload)
		if !ok {
			t.Fatalf("typing event payload = %T, want PresenceUpdatePayload", evt.Payload)
		}
		if payload.UserID != bob.ID || payload.Status != "typing" || payload.Mode != "replying" || payload.Board != "general" {
			t.Fatalf("typing payload = %+v, want bob typing on general", payload)
		}
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for typing presence event")
	}
	...
	if len(globalOnline) != 1 || globalOnline[0].UserID != bob.ID || globalOnline[0].Status != "active" || globalOnline[0].Mode != "reading" || globalOnline[0].LocationLabel != "General board" {
		t.Fatalf("typing should not overwrite persisted online presence, got %+v", globalOnline)
	}
	var persistedStatus, persistedMode string
	if err := c.DB.QueryRow(testRebind(`SELECT status, mode FROM user_presence_sessions WHERE user_id=? AND session_id=?`), bob.ID, "web").Scan(&persistedStatus, &persistedMode); err != nil {
		t.Fatalf("read persisted presence: %v", err)
	}
	if persistedStatus != "active" || persistedMode != "reading" {
		t.Fatalf("persisted presence = %s/%s, want active/reading", persistedStatus, persistedMode)
	}
	...
	exec(t, c, carol, proto.CmdSetPresence, proto.SetPresencePayload{Status: "typing"})
	notifs, err := c.ListNotifications(alice.ID, 10, 0, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(notifs) != 0 {
		t.Fatalf("typing should not satisfy login watches, got %+v", notifs)
	}
	var carolRows int
	if err := c.DB.QueryRow(testRebind(`SELECT COUNT(*) FROM user_presence_sessions WHERE user_id=?`), carol.ID).Scan(&carolRows); err != nil {
		t.Fatalf("count carol presence rows: %v", err)
	}
	if carolRows != 0 {
		t.Fatalf("typing-only user_presence_sessions rows = %d, want 0", carolRows)
	}
}
...
func TestMemoryChatStoreBacksCommandChatAndRecentReads(t *testing.T) {
	chatStore := core.NewMemoryChatStore()
	presenceStore := core.NewMemoryPresenceStore()
	c, cancel := newTestCoreWithOptions(t, core.WithChatStore(chatStore), core.WithPresenceStore(presenceStore))
	defer cancel()
	...
	first := exec(t, c, alice, proto.CmdSendChatLine, proto.SendChatLinePayload{
		Room: "Study-Room",
		Text: "  meet at the terminal  ",
	})
	second := exec(t, c, alice, proto.CmdSendChatLine, proto.SendChatLinePayload{
		Room: "study-room",
		Text: "bring notes",
	})
	...
	if len(lines) != 2 || gotLines[first.ID] != "meet at the terminal" || gotLines[second.ID] != "bring notes" {
		t.Fatalf("expected memory-backed recent chat lines, got %+v", lines)
	}
	rooms, err := c.ListChatRooms()
	if err != nil {
		t.Fatal(err)
	...
	if studyRoom == nil || studyRoom.Name != "Study Room" || studyRoom.LineCount != 2 || studyRoom.CreatedBy != alice.ID {
		t.Fatalf("expected memory-backed study room metadata, got rooms=%+v", rooms)
	}
	if studyRoom.OnlineUsers != 1 {
		t.Fatalf("expected memory-backed study room online count = 1, got %+v", studyRoom)
	}
	roster, err := c.ListChatOnlineUsers(alice.ID, "study-room", 10, 0)
	if err != nil {
		t.Fatal(err)
	...
	if len(roster) != 1 || roster[0].Name != "bob" || roster[0].Mode != "chat" || roster[0].LocationLabel != "study-room" {
		t.Fatalf("expected memory-backed chat roster, got %+v", roster)
	}
	var sqlChatLines int
	if err := c.DB.QueryRow(`SELECT COUNT(*) FROM chat_lines`).Scan(&sqlChatLines); err != nil {
		t.Fatal(err)
	}
	if sqlChatLines != 0 {
		t.Fatalf("memory chat store wrote %d SQL chat_lines rows, want 0", sqlChatLines)
	}
	var sqlStudyRooms int
	if err := c.DB.QueryRow(testRebind(`SELECT COUNT(*) FROM chat_rooms WHERE id=?`), "study-room").Scan(&sqlStudyRooms); err != nil {
		t.Fatal(err)
	}
	if sqlStudyRooms != 0 {
		t.Fatalf("memory chat store wrote %d SQL study-room rows, want 0", sqlStudyRooms)
	}
	var sqlPresenceRows int
	if err := c.DB.QueryRow(testRebind(`SELECT COUNT(*) FROM user_presence_sessions WHERE user_id=?`), bob.ID).Scan(&sqlPresenceRows); err != nil {
		t.Fatal(err)
	}
	if sqlPresenceRows != 0 {
		t.Fatalf("memory presence store wrote %d SQL session rows, want 0", sqlPresenceRows)
	}
}
...
func TestReactionWritesStayOffDurableEventLog(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	alice := registerAndGetUser(t, c, "alice", "pw")
	bob := registerAndGetUser(t, c, "bob", "pw")
	...
	sub := c.Bus.Subscribe([]string{"thread:" + thread.ID})
	...
	ack := exec(t, c, alice, proto.CmdReactPost, proto.ReactPostPayload{Post: posts[0].ID, Emoji: "heart"})
	if ack == nil || ack.ID != posts[0].ID || ack.Seq != 0 {
		t.Fatalf("reaction ack = %+v, want post id with no durable seq", ack)
	}
	headAfter, err := c.Head()
	if err != nil {
		t.Fatalf("head after reaction: %v", err)
	}
	if headAfter != headBefore {
		t.Fatalf("head after reaction = %d, want unchanged %d", headAfter, headBefore)
	}
	live := waitForTestEvent(t, sub, proto.EvtPostReacted)
	if live.Seq != 0 {
		t.Fatalf("live reaction seq = %d, want non-durable event", live.Seq)
	}
	payload, ok := live.Payload.(*proto.PostReactedPayload)
	if !ok || payload.PostID != posts[0].ID || payload.ReactionCount != 1 || payload.User != alice.Name {
		t.Fatalf("live reaction payload = %#v", live.Payload)
	}
	replayed, err := c.Replay(headBefore, []string{"thread:" + thread.ID}, 10)
	if err != nil {
		t.Fatalf("replay after reaction: %v", err)
	}
	for _, evt := range replayed {
		if evt.Kind == proto.EvtPostReacted || evt.Kind == proto.EvtPostUnreacted {
			t.Fatalf("replayed unordered reaction event %+v, want no durable reaction replay", evt)
		}
	}
	posts, err = c.ListPosts(thread.ID, 10, 0)
	if err != nil {
	...
	if posts[0].ReactionCount != 1 {
		t.Fatalf("reaction count = %d, want 1", posts[0].ReactionCount)
	}
	...
	if len(posts) != 1 || posts[0].ReactionCount != 1 {
		t.Fatalf("posts after rebuild = %+v, want reaction side-store restored", posts)
	}
	users, err := c.ListUserRankings(10, 0)
	if err != nil {
		t.Fatal(err)
	...
	foundBob := false
	for _, user := range users {
		if user.Name == bob.Name {
			foundBob = true
			if user.ReactionsReceived != 1 {
				t.Fatalf("bob reactions received after rebuild = %d, want 1", user.ReactionsReceived)
			}
		}
	}
	if !foundBob {
		t.Fatalf("bob missing from user rankings after rebuild: %+v", users)
	}
}

func TestPollVotesStayOffDurableEventLog(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	alice := registerAndGetUser(t, c, "alice", "pw")
	bob := registerAndGetUser(t, c, "bob", "pw")
	...
	if len(fullPoll.Options) != 2 {
		t.Fatalf("poll options = %d, want 2", len(fullPoll.Options))
	}
	optionID := fullPoll.Options[0].ID
	sub := c.Bus.Subscribe([]string{"thread:" + thread.ID})
	defer c.Bus.Unsubscribe(sub)
	headBefore, err := c.Head()
	if err != nil {
		t.Fatalf("head before vote: %v", err)
	}

	ack := exec(t, c, bob, proto.CmdVotePoll, proto.VotePollPayload{Poll: poll.ID, Option: optionID})
	if ack == nil || ack.ID != poll.ID || ack.Seq != 0 {
		t.Fatalf("vote ack = %+v, want poll id with no durable seq", ack)
	}
	headAfter, err := c.Head()
	if err != nil {
		t.Fatalf("head after vote: %v", err)
	}
	if headAfter != headBefore {
		t.Fatalf("head after vote = %d, want unchanged %d", headAfter, headBefore)
	}
	live := waitForTestEvent(t, sub, proto.EvtPollVoted)
	if live.Seq != 0 {
		t.Fatalf("live poll vote seq = %d, want non-durable event", live.Seq)
	}
	payload, ok := live.Payload.(*proto.PollVotedPayload)
	if !ok || payload.Poll != poll.ID || payload.Option != optionID || payload.User != bob.Name {
		t.Fatalf("live poll vote payload = %#v", live.Payload)
	}
	replayed, err := c.Replay(headBefore, []string{"thread:" + thread.ID}, 10)
	if err != nil {
		t.Fatalf("replay after vote: %v", err)
	}
	for _, evt := range replayed {
		if evt.Kind == proto.EvtPollVoted {
			t.Fatalf("replayed unordered poll vote event %+v, want no durable vote replay", evt)
		}
	}
	fullPoll, err = c.GetPoll(poll.ID, bob.ID)
	if err != nil {
	...
	if fullPoll.Voted != optionID || fullPoll.Options[0].VoteCount != 1 {
		t.Fatalf("poll after vote = %+v, want viewer vote and count", fullPoll)
	}

	if err := c.RebuildProjectionsFromEventLog(0); err != nil {
		t.Fatalf("rebuild projections: %v", err)
	}
	rebuiltPoll, err := c.GetPollByPostID(posts[0].ID)
	if err != nil {
		t.Fatal(err)
	}
	if rebuiltPoll == nil {
		t.Fatal("expected rebuilt poll")
	}
	fullPoll, err = c.GetPoll(rebuiltPoll.ID, bob.ID)
	if err != nil {
		t.Fatal(err)
	}
	if fullPoll == nil || fullPoll.Voted == "" {
		t.Fatalf("poll after rebuild = %+v, want vote side-store restored", fullPoll)
	}
	votedCount := 0
	for _, option := range fullPoll.Options {
		if option.ID == fullPoll.Voted {
			votedCount = option.VoteCount
		}
	}
	if votedCount != 1 {
		t.Fatalf("rebuilt voted option count = %d in %+v, want 1", votedCount, fullPoll.Options)
	}
}

func TestCounterSideStoreLossIsNotRecreatedByOrderedReplay(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	alice := registerAndGetUser(t, c, "alice", "pw")
	bob := registerAndGetUser(t, c, "bob", "pw")
	...
	if fullPoll == nil || len(fullPoll.Options) != 2 {
		t.Fatalf("poll = %+v, want two options", fullPoll)
	}

	exec(t, c, bob, proto.CmdReactPost, proto.ReactPostPayload{Post: posts[0].ID, Emoji: "heart"})
	exec(t, c, bob, proto.CmdVotePoll, proto.VotePollPayload{Poll: poll.ID, Option: fullPoll.Options[0].ID})
	posts, err = c.ListPosts(thread.ID, 10, 0)
	if err != nil {
		t.Fatal(err)
	}
	if posts[0].ReactionCount != 1 {
		t.Fatalf("reaction count before side-store loss = %d, want 1", posts[0].ReactionCount)
	}
	voted, err := c.GetPoll(poll.ID, bob.ID)
	if err != nil {
		t.Fatal(err)
	}
	if voted.Voted == "" || voted.Options[0].VoteCount != 1 {
		t.Fatalf("poll before side-store loss = %+v, want bob vote", voted)
	}

	if _, err := c.DB.Exec(testRebind(`DELETE FROM post_reactions WHERE post_id=?`), posts[0].ID); err != nil {
		t.Fatalf("delete post_reactions: %v", err)
	}
	if _, err := c.DB.Exec(testRebind(`DELETE FROM poll_votes WHERE poll_id=?`), poll.ID); err != nil {
		t.Fatalf("delete poll_votes: %v", err)
	}
	if _, err := c.DB.Exec(testRebind(`UPDATE user_activity SET reactions_recv=7 WHERE user_id=?`), alice.ID); err != nil {
		t.Fatalf("poison reactions_recv: %v", err)
	}

	if err := c.RebuildProjectionsFromEventLog(0); err != nil {
		t.Fatalf("rebuild projections after side-store loss: %v", err)
	}
	posts, err = c.ListPosts(thread.ID, 10, 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(posts) != 1 || posts[0].ReactionCount != 0 {
		t.Fatalf("posts after side-store loss rebuild = %+v, want no fabricated reaction count", posts)
	}
	rebuiltPoll, err := c.GetPollByPostID(posts[0].ID)
	if err != nil {
		t.Fatal(err)
	...
	if rebuiltPoll == nil {
		t.Fatal("expected rebuilt poll structure from durable post event")
	}
	rebuiltVote, err := c.GetPoll(rebuiltPoll.ID, bob.ID)
	if err != nil {
		t.Fatal(err)
	}
	if rebuiltVote.Voted != "" {
		t.Fatalf("poll after side-store loss rebuild = %+v, want no fabricated viewer vote", rebuiltVote)
	}
	for _, option := range rebuiltVote.Options {
		if option.VoteCount != 0 {
			t.Fatalf("poll options after side-store loss rebuild = %+v, want zero vote counts", rebuiltVote.Options)
		}
	}
	users, err := c.ListUserRankings(10, 0)
	if err != nil {
	...
	for _, user := range users {
		if user.Name == alice.Name && user.ReactionsReceived != 0 {
			t.Fatalf("alice reactions received after side-store loss rebuild = %d, want recomputed zero", user.ReactionsReceived)
		}
	}
}

func TestCounterCheckpointRebuildsCoarseCountsFromDurableEvent(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	alice := registerAndGetUser(t, c, "alice", "pw")
	bob := registerAndGetUser(t, c, "bob", "pw")
	...
	sourceHead, err := c.Head()
	if err != nil {
		t.Fatal(err)
	}
	checkpointSeq, err := c.CheckpointCounters(context.Background())
	if err != nil {
		t.Fatalf("CheckpointCounters: %v", err)
	}
	if checkpointSeq <= sourceHead {
		t.Fatalf("checkpoint seq = %d, want after source head %d", checkpointSeq, sourceHead)
	}
	events, err := c.Replay(checkpointSeq-1, nil, 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 1 || events[0].Kind != proto.EvtCounterCheckpointed {
		t.Fatalf("checkpoint replay events = %+v, want counter checkpoint", events)
	}
	payload, ok := events[0].Payload.(*proto.CounterCheckpointPayload)
	if !ok {
		t.Fatalf("checkpoint payload type = %T, want *proto.CounterCheckpointPayload", events[0].Payload)
	}
	if !payload.Complete || payload.SourceHeadSeq != sourceHead {
		t.Fatalf("checkpoint payload = %+v, want complete snapshot at source head %d", payload, sourceHead)
	}

	assertCounterCheckpointCount(t, c.DB, "post.reactions", posts[0].ID, 1)
	assertCounterCheckpointCount(t, c.DB, "poll.option_votes", fullPoll.Options[0].ID, 1)
	assertCounterCheckpointCount(t, c.DB, "poll.option_votes", fullPoll.Options[1].ID, 0)
	...
	if _, err := c.DB.Exec(`DELETE FROM counter_checkpoints`); err != nil {
		t.Fatalf("delete counter_checkpoints: %v", err)
	}
	if err := c.RebuildProjectionsFromEventLog(0); err != nil {
		t.Fatalf("rebuild projections: %v", err)
	}
...
func TestCounterCheckpointSchedulerEmitsDurableCheckpoint(t *testing.T) {
	c, cancel := newTestCore(t)
	defer cancel()
	alice := registerAndGetUser(t, c, "alice", "pw")
	bob := registerAndGetUser(t, c, "bob", "pw")
	...
	thread := exec(t, c, alice, proto.CmdCreateThread, proto.CreateThreadPayload{
		Board: "general",
		Title: "Scheduled counter checkpoint",
		Body:  "[poll]\nSchedule checkpoint?\nYes\nNo\n[/poll]",
	})
	posts, err := c.ListPosts(thread.ID, 10, 0)
	if err != nil {
	...
	defer stopScheduler()
	c.StartBackgroundWorkerWithConfig(schedulerCtx, core.BackgroundWorkerConfig{
		CounterCheckpointInterval: 20 * time.Millisecond,
	})

	var checkpoint *proto.Event
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		events, err := c.Replay(sourceHead, nil, 20)
		if err != nil {
			t.Fatal(err)
		}
		for _, evt := range events {
			if evt.Kind == proto.EvtCounterCheckpointed {
				checkpoint = evt
				break
			}
		}
	...
	if checkpoint == nil {
		t.Fatal("scheduled counter checkpoint did not appear")
	}
	payload, ok := checkpoint.Payload.(*proto.CounterCheckpointPayload)
	if !ok {
		t.Fatalf("checkpoint payload type = %T, want *proto.CounterCheckpointPayload", checkpoint.Payload)
	}
	if payload.SourceHeadSeq != sourceHead {
		t.Fatalf("checkpoint source head = %d, want %d", payload.SourceHeadSeq, sourceHead)
	}
}

func assertCounterCheckpointCount(t *testing.T, db *sql.DB, kind, targetID string, want int) {
	t.Helper()
	var got int
	if err := db.QueryRow(testRebind(`SELECT count FROM counter_checkpoints WHERE counter_kind=? AND target_id=?`), kind, targetID).Scan(&got); err != nil {
		t.Fatalf("counter checkpoint %s/%s: %v", kind, targetID, err)
	}
	if got != want {
		t.Fatalf("counter checkpoint %s/%s count = %d, want %d", kind, targetID, got, want)
	}
}
