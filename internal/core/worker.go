...
const (
	attachmentBlobStagingPruneInterval = 15 * time.Minute
	attachmentBlobStagingPruneLimit    = 500
)

type BackgroundWorkerConfig struct {
	AutoStats                 bool
	CounterCheckpointInterval time.Duration
}
...
func (c *Core) StartBackgroundWorker(ctx context.Context, autoStats bool) {
	c.StartBackgroundWorkerWithConfig(ctx, BackgroundWorkerConfig{AutoStats: autoStats})
}
...
func (c *Core) StartBackgroundWorkerWithConfig(ctx context.Context, config BackgroundWorkerConfig) {
	if currentSQLFlavor != postgresFlavor {
		...
		if config.CounterCheckpointInterval > 0 {
			go c.runCounterCheckpointScheduler(ctx, config.CounterCheckpointInterval)
		}
		go c.runAttachmentBlobStagingPruner(ctx, attachmentBlobStagingPruneInterval)
		return
	}
	go c.runLeaderElectedWorker(ctx, config)
}
...
// then re-contends.
func (c *Core) runLeaderElectedWorker(ctx context.Context, config BackgroundWorkerConfig) {
	for ctx.Err() == nil {
		...
		c.serveAsLeader(ctx, conn, config)
...
func (c *Core) serveAsLeader(ctx context.Context, conn *sql.Conn, config BackgroundWorkerConfig) {
	...
	if config.CounterCheckpointInterval > 0 {
		go c.runCounterCheckpointScheduler(leaderCtx, config.CounterCheckpointInterval)
	}
	go c.runAttachmentBlobStagingPruner(leaderCtx, attachmentBlobStagingPruneInterval)
...
func (c *Core) runCounterCheckpointScheduler(ctx context.Context, interval time.Duration) {
	if interval <= 0 {
		return
	}
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			seq, err := c.CheckpointCounters(ctx)
			if err != nil {
				if ctx.Err() == nil {
					slog.Warn("automatic counter checkpoint failed", "err", err)
				}
				continue
			}
			slog.Info("automatic counter checkpoint ensured", "seq", seq)
		}
	}
}

func (c *Core) runAttachmentBlobStagingPruner(ctx context.Context, interval time.Duration) {
	if interval <= 0 {
		return
	}
	prune := func() {
		deleted, err := c.PruneExpiredAttachmentBlobStaging(attachmentBlobStagingPruneLimit)
		if err != nil {
			if ctx.Err() == nil {
				slog.Warn("attachment blob staging prune failed", "err", err)
			}
			return
		}
		if deleted > 0 {
			slog.Info("attachment blob staging pruned expired rows", "deleted", deleted)
		}
	}
	prune()
	ticker := time.NewTicker(interval)
