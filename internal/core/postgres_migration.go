...
func MigrateSQLiteToPostgres(ctx context.Context, sqlitePath, pgDSN string) error {
	...
	tableCopies := []struct {
		...
		{src: "favorite_folders", dst: "favorite_folders", cols: []string{"id", "user_id", "parent_id", "name", "position", "created_at", "updated_at"}},
		{src: "board_favorites", dst: "board_favorites", cols: []string{"user_id", "board_id", "folder_id", "position", "created_at", "updated_at"}},
		{src: "board_zaps", dst: "board_zaps", cols: []string{"user_id", "board_id", "created_at", "updated_at"}},
		{src: "board_settings", dst: "board_settings", cols: []string{"board_id", "anonymous_allowed", "read_only", "no_reply", "attachments_allowed", "mail_in_allowed", "relay_enabled", "member_read_mode", "member_post_mode", "stats_excluded", "zap_allowed", "updated_at"}},
		{src: "board_moderators", dst: "board_moderators", cols: []string{"board_id", "user_id", "position", "created_at", "updated_at"}},
		{src: "board_moderator_terms", dst: "board_moderator_terms", cols: []string{"board_id", "user_id", "started_at", "ended_at", "appointed_by", "removed_by", "position", "created_at", "updated_at"}},
		{src: "board_members", dst: "board_members", cols: []string{"board_id", "user_id", "title", "position", "can_manage_members", "can_curate", "can_moderate_posts", "can_moderate_threads", "can_announce", "can_manage_polls", "can_set_board_settings", "created_at", "updated_at"}},
		{src: "board_member_applications", dst: "board_member_applications", cols: []string{"id", "board_id", "user_id", "status", "note", "title", "reviewer_id", "review_note", "created_at", "updated_at", "reviewed_at"}},
		{src: "board_member_requirements", dst: "board_member_requirements", cols: []string{"board_id", "min_login_count", "min_post_count", "min_trust_level", "min_score", "min_board_post_count", "min_board_original_post_count", "min_board_digest_count", "min_board_mark_count", "max_members", "approval_mode", "updated_at"}},
		{src: "board_read_markers", dst: "board_read_markers", cols: []string{"user_id", "board_id", "last_seq", "previous_seq", "updated_at"}},
		{src: "thread_read_markers", dst: "thread_read_markers", cols: []string{"user_id", "thread_id", "last_seq", "previous_seq", "updated_at"}},
		...
		{src: "mail_group_deletions", dst: "mail_group_deletions", cols: []string{"event_id", "owner_id", "group_id", "deleted_at"}},
		...
		{src: "post_reaction_count_shards", dst: "post_reaction_count_shards", cols: []string{"post_id", "shard", "count_value", "updated_at"}},
		{src: "poll_vote_count_shards", dst: "poll_vote_count_shards", cols: []string{"poll_id", "option_id", "shard", "count_value", "updated_at"}},
		...
		{src: "digest_entry_removals", dst: "digest_entry_removals", cols: []string{"id", "board_id", "kind", "removed_by", "removed_at"}},
		{src: "digest_path_mutations", dst: "digest_path_mutations", cols: []string{"event_id", "action", "board_id", "kind", "from_path", "to_path", "actor_id", "ts", "count"}},
		...
		{src: "community_stat_history", dst: "community_stat_history", cols: []string{"day", "snapshot_at", "total_users", "total_boards", "total_threads", "total_posts", "total_reactions", "total_mail", "total_direct_messages", "total_logins", "total_logouts", "total_web_logins", "total_web_logouts", "total_guest_logins", "total_guest_logouts", "total_online_seconds", "online_users", "online_guests", "max_online_users", "max_online_at", "max_online_guests", "max_online_guests_at", "head_seq"}},
		{src: "derived_view_watermarks", dst: "derived_view_watermarks", cols: []string{"view_name", "applied_seq", "updated_at"}},
		...
		{src: "processed_commands_v2", dst: "processed_commands_v2", cols: []string{"partition_kind", "partition_key", "actor_id", "cid", "command_hash", "result_json", "processed_at"}},
		{src: "command_log_receipts", dst: "command_log_receipts", cols: []string{"partition_kind", "partition_key", "actor_id", "cid", "command_offset", "status", "error_json", "updated_at"}},
		{src: "hot_thread_splits", dst: "hot_thread_splits", cols: []string{"thread_id", "shards", "updated_at"}},
	}
	...
	if err := seedPostgresPartitionOffsets(ctx, tx); err != nil {
		return fmt.Errorf("seed partition offsets: %w", err)
	}
...
func copyEvents(ctx context.Context, tx *sql.Tx, src *sql.DB) (int64, error) {
	rows, err := src.QueryContext(ctx, `SELECT seq, id, kind, payload, ts, scopes, partition_kind, partition_key, partition_offset FROM events ORDER BY seq`)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&seq, &id, &kind, &payload, &ts, &scopes, &partitionKind, &partitionKey, &partitionOffset); err != nil {
			return 0, err
		}
		...
		_, err := tx.ExecContext(
			ctx,
			`INSERT INTO events (seq, id, kind, payload, created_at, partition_kind, partition_key, partition_offset)
			 ...
			 ON CONFLICT (seq) DO UPDATE
			   SET id=EXCLUDED.id, kind=EXCLUDED.kind, payload=EXCLUDED.payload, created_at=EXCLUDED.created_at,
			       partition_kind=EXCLUDED.partition_kind, partition_key=EXCLUDED.partition_key, partition_offset=EXCLUDED.partition_offset`,
			seq,
			id,
			kind,
			payloadText,
...
func seedPostgresPartitionOffsets(ctx context.Context, tx *sql.Tx) error {
