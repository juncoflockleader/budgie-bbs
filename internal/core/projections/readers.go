...
func getCommunityStatsCurrent(db sqlLike) (*CommunityStats, error) {
	...
	err := QQueryRow(db,
		`SELECT
		    ...
		    (SELECT `+postReactionAggregateSumSQL("prc", "p.id")+`
		       FROM posts p
		       JOIN threads t ON t.id=p.thread
...
func applyCommunityStatsMaxOnline(db sqlLike, stats *CommunityStats) error {
	var maxOnline int
	var maxAt int64
	err := QQueryRow(db,
...
func CommunityStatsSnapshotRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM community_stats_snapshot`).Scan(&count)
	return count, err
}
...
func GetCommunityStatsSnapshot(db *sql.DB) (*CommunityStats, error) {
	stats := &CommunityStats{}
	err := QQueryRow(db,
		`SELECT total_users, total_boards, total_threads, total_posts,
...
func BoardRankingStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM board_ranking_stats`).Scan(&count)
	return count, err
}
...
func ListBoardRankingsMaterialized(db *sql.DB, viewerID string, includePrivate bool, limit, offset int) ([]BoardRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT b.id, b.name, b.description,
		  ...
		  ORDER BY r.post_count DESC, r.thread_count DESC, r.last_seq DESC, b.name
		  LIMIT ? OFFSET ?`,
		onlineCutoff, boolInt(includePrivate), viewerID, viewerID, limit, offset,
	)
	if err != nil {
...
func ListThreadRankings(db *sql.DB, viewerID string, includePrivate bool, boardID string, limit, offset int) ([]ThreadRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT t.id, t.board, b.name, t.title, t.author, COALESCE(t.author_id, ''),
		        ...
		        `+postReactionAggregateSumSQL("prc", "p.id")+` AS reaction_count,
...
func ThreadRankingStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM thread_ranking_stats`).Scan(&count)
	return count, err
}
...
func ReplyRankingPostsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM reply_ranking_posts`).Scan(&count)
	return count, err
}
...
func ListReplyRankingsMaterialized(db *sql.DB, viewerID string, includePrivate bool, limit, offset int) ([]ReplyRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT p.id, t.id, t.board, b.name, t.title, p.author, COALESCE(p.author_id, ''),
		        COALESCE(p.body, '') AS body,
		        r.created_seq, r.created_at
		   FROM reply_ranking_posts r
...
func ListUserRankings(db *sql.DB, limit, offset int) ([]UserRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT u.id, u.name, u.role,
		        ...
		        `+postReactionAggregateSumSQL("prc", "p.id")+` AS reactions_received,
...
func UserRankingStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM user_ranking_stats`).Scan(&count)
	return count, err
}
...
func ListUserRankingsMaterialized(db *sql.DB, limit, offset int) ([]UserRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT u.id, u.name, u.role,
		        r.posts_created,
		        r.reactions_received,
		        r.login_count,
		        r.total_online_seconds,
		        r.trust_level
		   FROM user_ranking_stats r
		   JOIN users u ON u.id=r.user_id
		  ORDER BY r.posts_created DESC,
		           r.reactions_received DESC,
		           r.login_count DESC,
		           r.total_online_seconds DESC,
		           u.name
		  LIMIT ? OFFSET ?`,
		limit, offset,
	)
...
func BlessingRankingStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM blessing_ranking_stats`).Scan(&count)
	return count, err
}
...
func ListBlessingRankingsMaterialized(db *sql.DB, limit, offset int) ([]BlessingRanking, error) {
	...
	rows, err := QQuery(db,
		`SELECT u.id, u.name, r.blessing_count, r.last_blessed_at
		   FROM blessing_ranking_stats r
		   JOIN users u ON u.id=r.user_id
		  ORDER BY r.blessing_count DESC, r.last_blessed_at DESC, u.name
		  LIMIT ? OFFSET ?`,
		limit, offset,
	)
	if err != nil {
...
func ArchiveRankingStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM archive_ranking_stats`).Scan(&count)
	return count, err
}
...
func ListArchiveRankingsMaterialized(db *sql.DB, viewerID string, includePrivate bool, kind string, limit, offset int) ([]ArchiveRanking, error) {
	...
	kind = strings.TrimSpace(kind)
	rows, err := QQuery(db,
		`SELECT r.board_id, b.name, r.kind, r.path,
		        r.entry_count,
		        r.edited_count,
		        r.last_updated_at
		   FROM archive_ranking_stats r
		   JOIN boards b ON b.id=r.board_id
		   LEFT JOIN board_settings s ON s.board_id=b.id
		  WHERE (? = '' OR r.kind = ?)
		    AND COALESCE(s.stats_excluded, 0)=0
		  ...
		  ORDER BY r.entry_count DESC, r.edited_count DESC, r.last_updated_at DESC, b.name, r.path
		  LIMIT ? OFFSET ?`,
		kind, kind, boolInt(includePrivate), viewerID, viewerID, limit, offset,
	)
	if err != nil {
...
func ListDigestEntries(db *sql.DB, boardID, kind, path string, limit, offset int) ([]DigestEntry, error) {
	...
	rows, err := QQuery(db,
		`SELECT d.id, d.board_id, b.name, d.target_kind, d.target_id, d.kind, d.title, d.path, d.note, d.body_edited,
		        ...
		        COALESCE(CASE WHEN d.target_kind='thread' THEN tt.id ELSE pt.id END, '') AS thread_id,
		        COALESCE(CASE WHEN d.target_kind='post' THEN p.id ELSE '' END, '') AS post_id,
		        COALESCE(CASE WHEN d.target_kind='thread' THEN tt.author ELSE p.author END, '') AS author,
		        COALESCE(CASE
...
func BoardSummaryStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM board_summary_stats`).Scan(&count)
	return count, err
}
	...
	rows, err := QQuery(db,
		`WITH board_state AS (
		    SELECT b.id, b.name, b.description,
		           ...
		           COALESCE(m.last_seq, 0) AS read_seq,
		           s.created_at,
		           CASE WHEN s.created_at > 0 AND s.created_at >= ? THEN 1 ELSE 0 END AS new_board,
		           s.last_seq,
		           s.thread_count,
		           s.post_count,
		           COALESCE((SELECT COUNT(DISTINCT ups.user_id)
		                     ...
		                     WHERE ups.board_id = b.id
		                       AND ups.last_seen >= ?
		                       AND LOWER(ups.status) NOT IN ('offline', 'invisible', 'cloak', 'cloaked')), 0) AS online_users,
		           CASE WHEN bz.board_id IS NOT NULL AND COALESCE(bs.zap_allowed, 1) != 0 THEN 1 ELSE 0 END AS zapped,
		           COALESCE((SELECT COUNT(*) FROM threads t WHERE t.board = b.id AND t.last_seq > COALESCE(m.last_seq, 0)), 0) AS unread_threads,
		           COALESCE((SELECT COUNT(*)
		                     FROM posts p
		                     JOIN threads t ON t.id = p.thread
		                     WHERE t.board = b.id
		                       ...
		                       AND p.redacted = 0), 0) AS unread_posts,
		           s.moderator_count
		      FROM board_summary_stats s
		      JOIN boards b ON b.id = s.board_id
		      LEFT JOIN board_favorites f ON f.board_id = b.id AND f.user_id = ?
		      LEFT JOIN board_zaps bz ON bz.board_id = b.id AND bz.user_id = ?
		      LEFT JOIN board_read_markers m ON m.board_id = b.id AND m.user_id = ?
		      LEFT JOIN board_settings bs ON bs.board_id = b.id
		)
		SELECT id, name, description, favorite, unread_threads, unread_posts,
		       thread_count, post_count, online_users, last_seq, read_seq, created_at, new_board, zapped,
		       COALESCE((SELECT anonymous_allowed FROM board_settings WHERE board_id=id), 0),
...
func UnreadThreadSummaryStatsRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM unread_thread_summary_stats`).Scan(&count)
	return count, err
}
...
func ListUnreadThreadSummariesMaterialized(db *sql.DB, userID string, includePrivate bool, favoritesOnly bool, folderID string, limit, offset int) ([]ThreadSummary, error) {
	...
	rows, err := QQuery(db,
		...
		thread_state AS (
		    SELECT s.thread_id AS id, s.board_id AS board, b.name AS board_name,
		           s.author, COALESCE(s.author_id,'') AS author_id, s.title, s.locked, s.post_count,
		           s.last_seq, s.created_ts, s.created_at, s.updated_at,
		           CASE
		             WHEN COALESCE(trm.last_seq, 0) > COALESCE(brm.last_seq, 0) THEN COALESCE(trm.last_seq, 0)
		             ELSE COALESCE(brm.last_seq, 0)
		           END AS read_seq
		      FROM unread_thread_summary_stats s
		      JOIN allowed_boards ab ON ab.id = s.board_id
		      JOIN boards b ON b.id = s.board_id
		      LEFT JOIN board_read_markers brm ON brm.board_id = s.board_id AND brm.user_id = ?
		      LEFT JOIN thread_read_markers trm ON trm.thread_id = s.thread_id AND trm.user_id = ?
		)
		SELECT id, board, board_name, author, author_id, title, locked, post_count, last_seq, created_ts, created_at, updated_at, read_seq,
		       unread_posts,
		       first_unread_post
...
func ListPosts(db *sql.DB, threadID string, limit, offset int) ([]Post, error) {
	rows, err := QQuery(db,
		`SELECT id, thread, author, COALESCE(author_id,''), body, COALESCE(signature,''), content_type, COALESCE(reply_to,''), version, redacted,
		        COALESCE((SELECT SUM(count_value) FROM post_reaction_count_shards WHERE post_id=posts.id), (SELECT COUNT(*) FROM post_reactions WHERE post_id=posts.id), 0),
...
func ResidentFeedMaterializedRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM resident_feed_posts`).Scan(&count)
	return count, err
}
...
func ListResidentBoardPostsMaterialized(db *sql.DB, userID string, limit, offset int) ([]Post, error) {
	...
	rows, err := QQuery(db,
		`SELECT p.id, p.thread, t.board, b.name AS board_name, t.title AS thread_title,
		        ...
		        COALESCE((SELECT SUM(count_value) FROM post_reaction_count_shards WHERE post_id=p.id), (SELECT COUNT(*) FROM post_reactions WHERE post_id=p.id), 0),
		        p.created_seq, p.updated_seq, p.created_at, p.updated_at
		   FROM resident_feed_posts f
		   JOIN threads t ON t.id=f.thread_id AND t.board=f.board_id
		   JOIN boards b ON b.id=t.board
		   JOIN posts p ON p.id=f.post_id AND p.thread=t.id
		   JOIN board_members bm ON bm.board_id=f.board_id AND bm.user_id=?
		  WHERE p.redacted=0
		  ORDER BY f.created_seq DESC
		  LIMIT ? OFFSET ?`,
		userID, limit, offset,
	)
	if err != nil {
...
func ListLatestFeedPosts(db *sql.DB, viewerID string, includePrivate bool, limit, offset int) ([]Post, error) {
	...
	rows, err := QQuery(db,
		`SELECT p.id, p.thread, t.board, b.name AS board_name, t.title AS thread_title,
		  ...
		  ORDER BY p.created_seq DESC
		  LIMIT ? OFFSET ?`,
		viewerID, boolInt(includePrivate), viewerID, viewerID, limit, offset,
	)
	if err != nil {
		return nil, err
	}
...
func LatestFeedMaterializedRowCount(db *sql.DB) (int, error) {
	var count int
	err := QQueryRow(db, `SELECT COUNT(*) FROM latest_feed_posts`).Scan(&count)
	return count, err
}
...
func ReactionCount(db *sql.DB, postID string) (int, error) {
	if n, ok, err := reactionCountFromShards(db, postID); err != nil || ok {
		return n, err
	}
	var n int
...
func ReactionCountTx(tx *sql.Tx, postID string) (int, error) {
	if n, ok, err := reactionCountFromShards(tx, postID); err != nil || ok {
		return n, err
	}
	var n int
...
func reactionCountFromShards(queryable sqlLike, postID string) (int, bool, error) {
	var n sql.NullInt64
	err := QQueryRow(queryable, `SELECT SUM(count_value) FROM post_reaction_count_shards WHERE post_id=?`, postID).Scan(&n)
	if err != nil {
		return 0, false, err
	}
	if !n.Valid {
