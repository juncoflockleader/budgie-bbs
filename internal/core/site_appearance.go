...
func (c *Core) SiteAppearance() (*SiteAppearance, error) {
	...
	err := qQueryRow(c.DB,
		`SELECT site_title, tagline, banner_message, accent_color, default_theme, updated_at
		   FROM site_appearance_settings WHERE id='default'`,
	).Scan(&out.SiteTitle, &out.Tagline, &out.BannerMessage, &out.AccentColor, &out.DefaultTheme, &out.UpdatedAt)
...
func (c *Core) SetSiteAppearance(a SiteAppearance) (*SiteAppearance, error) {
	...
	if _, err := qExec(c.DB,
		`INSERT INTO site_appearance_settings (id, site_title, tagline, banner_message, accent_color, default_theme, updated_at)
		 VALUES ('default', ?, ?, ?, ?, ?, ?)
		 ON CONFLICT(id) DO UPDATE SET
		   site_title=excluded.site_title, tagline=excluded.tagline, banner_message=excluded.banner_message,
		   accent_color=excluded.accent_color, default_theme=excluded.default_theme, updated_at=excluded.updated_at`,
		title, tagline, banner, accent, theme, nowMS(),
	); err != nil {
		return nil, err
	}
