...
func NewKafkaEventRecord(topic string, record logmodel.BrokerEventRecord) (*kgo.Record, error) {
...
func DecodeKafkaEventRecord(record *kgo.Record) (logmodel.BrokerEventLogMessage, error) {
	return DecodeKafkaEventRecordWithOptions(record, KafkaEventRecordDecodeOptions{})
}

func DecodeKafkaEventRecordWithOptions(record *kgo.Record, options KafkaEventRecordDecodeOptions) (logmodel.BrokerEventLogMessage, error) {
	if record == nil {
		return logmodel.BrokerEventLogMessage{}, fmt.Errorf("kafka event log: nil record")
	}
	if record.Offset < 0 {
		return logmodel.BrokerEventLogMessage{}, fmt.Errorf("kafka event log: negative record offset %d", record.Offset)
	}
	...
	if len(record.Key) > 0 && string(record.Key) != LogicalPartitionKey(partition) {
		return logmodel.BrokerEventLogMessage{}, fmt.Errorf("kafka event log: record key %q does not match partition %s/%s",
