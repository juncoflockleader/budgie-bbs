...
3. **One replay surface, compatible cursors.** Every durable event has a global monotonic `seq` in v1. A client's baseline position is still that single integer, while partition-aware clients may also carry an optional cursor envelope with per-partition offsets. Live delivery, catch-up, and history are the same mechanism at different starting points.
...
Because the baseline position is a single `seq` cursor, every read tier is a variation of one operation — *"give me events after `N`"*. Partition-aware clients can include the optional cursor envelope; servers that do not understand it keep using `after`.
...
```json
{
  ...
  "eseq": null,
  "partitionKind": "board",       // optional partition cursor metadata
  "partitionKey": "tech",
  "partitionOffset": 318,
  "cursor": {
    "seq": 41021,
    "partitions": [{ "kind": "board", "key": "tech", "offset": 318 }]
  },
  "payload": { },
  "ts": 1716500000123
}
...
When the server is running with authoritative command-log submission, a
successful command can be accepted durably before it has materialized in
projections:
```json
{
  "kind": "ack",
  ...
  "result": {
    ...
    "status": "pending",
    "commandId": "c_01J8X...",
    "commandPartitionKind": "board",
    "commandPartitionKey": "tech",
    "commandOffset": 318
  },
  "error": null
}
```
Clients should treat this as durable acceptance, not completion of the
underlying mutation, and then observe the resulting event or projection update.
Flows that need the materialized result id before navigating or clearing local
draft state should poll `GET /api/v1/commands/{commandId}` with
`commandPartitionKind`, `commandPartitionKey`, and `commandOffset` from the ack.
REST alias helpers that receive the same pending HTTP ack envelope should follow
that polling path before exposing the mutation as complete to UI code.
Servers should return that pending ack before any legacy post-command readback
that would otherwise inspect stale local state.
The server validates that the requested partition and offset contain that exact
command id; stale or mismatched offsets return `404`. That read returns
`status: "pending"` until the writer commits the command,
`status: "retrying"` with the latest retryable writer error after an attempted
drain that did not commit the offset, `status: "applied"` with the stored `ack`
result after successful materialization, `status: "failed"` with the terminal
command error when the writer commits a non-retryable rejection, or
`status: "committed"` when the command offset is committed but no result/error
receipt is available. `applied` responses also include `committedOffset`; if it
is still lower than `commandOffset`, SQL materialization has succeeded and the
writer is still closing the command-log offset checkpoint.
...
```json
{
  ...
  "payload": {
    ...
    "headCursor": { "seq": 41020 },
    "capabilities": ["ws", "sse", "longpoll", "poll", "ansi-art", "edit", "redact", "partition-cursors"],
...
```json
{
  ...
  "payload": {
    ...
    "cursor": {
      "seq": 40990,
      "partitions": [{ "kind": "board", "key": "tech", "offset": 302 }]
    },
    "subscriptions": ["board:tech", "thread:88", "chat:lobby", "presence:global"]
  }
}
```

Partition-aware clients that want partition-native replay SHOULD keep `after`
as the scalar compatibility fallback and send the cursor envelope with
`partitions` only. Older servers ignore the cursor and use `after`; newer
servers replay from partition offsets.

The server replays from the supplied scalar or partition cursor for subscribed scopes, then attaches the live tail. While streaming, the server advances the delivered cursor and repairs scalar or per-partition gaps from the durable log before sending the next live event. A fresh client sends `"after": 0` (or omits it) to backfill from the projection rather than the full log — see §7.
...
No persistent handshake. Each request carries auth (§5) and the cursor as a query param. The equivalent of `welcome.head` is returned in response headers (`X-Log-Head` and, for partition-aware clients, `X-Log-Cursor`) on every read.
...
- **`seq`** is the global monotonic durable compatibility cursor. It only ever increases and is assigned at durable append time. **Ordering is by `seq`, never by `ts`, for scalar replay clients.**
- **`cursor`** is an optional durable cursor envelope: `{ "seq": N, "partitions": [{ "kind": "...", "key": "...", "offset": M }] }`. During v1 compatibility, scalar `seq` remains the default replay boundary when present for request backlog, while live WS/SSE streams also track partition offsets and use them to repair partition gaps. Partition-aware clients may send `after=N` plus a partition-only cursor to replay from per-partition offsets while preserving scalar fallback for older servers; returned batches remain compatibility-sorted by `seq`.
...
- **Idempotent retries.** Because HTTP retries are routine, the server deduplicates commands by command partition, actor, and `cid` within a window (default 10 min). Re-sending a command with a `cid` it already applied in the same partition returns the original `ack`, not a second effect. Clients SHOULD reuse the same `cid` when retrying a command they're unsure landed, and SHOULD still treat each pending command's `cid` as unique even though independent partitions can reuse one without colliding.
...
| `command_log_unavailable` | Command log append failed before durable acceptance | yes |
| `projection_stale` | Projection has not applied a requested minimum durable sequence | yes |
| `write_region_unavailable` | Regional proxy could not reach the authoritative write region | yes |
| `blob_staging_required` | Attachment writer could not see required staged blob bytes | yes |

HTTP transport maps these to status codes (`401`, `403`, `429`, `422`, `404`,
`409`, `425`, `502`, `503`) with the same JSON `error` body.
...
Regional deployments may configure an HTTP write-region proxy. In that mode,
safe methods (`GET`, `HEAD`, `OPTIONS`) continue to serve local reads, while
mutating `/api/v1/*` requests are reverse-proxied to the authoritative write
region with authorization headers, body, query string, and `X-Command-Id`
preserved. The proxy marks forwarded requests with
`X-Budgie-Write-Region-Routed: 1`, `X-Budgie-Write-Region`,
`X-Forwarded-Host`, and `X-Forwarded-Proto`. If the write region cannot be
reached, the regional node returns HTTP `502` with retryable
`write_region_unavailable`. WebSocket command frames are not proxied by this
HTTP mechanism; clients should use HTTP writes or an authoritative command-log
gateway when connected to a regional read node.

### Reads — events (the cursor surface)

```
GET /api/v1/events?after={seq}&cursor={json}&scope={s}&scope={s}&limit={n}&wait={sec}
```
- `wait=0` (or omit) → poll: returns immediately, possibly `{ "events": [], "head": N }`.
- `wait=30` → long-poll: holds up to 30s for new events.
- `cursor` is optional URL-encoded JSON using the envelope from §7. Servers fall back to `after` when it is absent or unsupported. If the cursor has no `seq` but has `partitions`, HTTP poll/long-poll replays each known event partition after its supplied offset.
- Response: `{ "events": [ <event envelopes> ], "head": 41021, "cursor": { "seq": 41021, "partitions": [...] }, "deliveredCursor": { "partitions": [...] } }`, plus `X-Log-Head: 41021`, `X-Log-Cursor` with the same head cursor envelope, and `X-Log-Delivered-Cursor` with the delivered cursor envelope. `cursor` remains the global log head for compatibility and freshness checks. Partition-aware clients that poll a scoped feed SHOULD persist `deliveredCursor`; when a scoped response advances partition offsets, the server may intentionally omit scalar `seq` from `deliveredCursor` so changing scopes later does not mark unrelated partitions as seen.
- Cacheable when `wait=0` and `after` is at/near a cache boundary.

SSE variant (push over HTTP):
```
GET /api/v1/events/stream?after={seq}&cursor={json}&scope={s}          Accept: text/event-stream
```
Each SSE `id:` field carries the `seq`, so the browser's automatic `Last-Event-ID` on reconnect resumes the scalar cursor with zero client logic. Partition-aware clients SHOULD also send the `cursor` query param on reconnect when available; the server uses it for initial replay and for per-partition live gap repair.
...
GET /api/v1/feed/latest?limit=&offset=
...
Cross-partition global projection reads, such as latest feeds, community
stats/history, rankings, plus search reads, are explicitly marked as eventually
consistent. They include additive metadata in the response body:

```json
{
  "meta": {
    "consistency": "eventual",
    "view": "rankings.boards",
    "headSeq": 41021,
    "appliedSeq": 41000,
    "lagEvents": 21
  }
...
The same values are mirrored in `X-Projection-Consistency`,
`X-Projection-View`,
`X-Projection-Head-Seq`, `X-Projection-Applied-Seq`, and
`X-Projection-Lag-Events` headers. Missing named-view watermarks resolve to the
current event head for backward compatibility with today's synchronous
projection engine; once an async processor owns a view, it advances that
watermark independently. Community stats use `community_stats` and may be
served from a worker-owned `community_stats_snapshot` row when available; the
processor may refresh without a new durable event because login, online-time,
and presence counters can change outside the durable ordered event path.
Clients that require read-your-writes can send `X-Budgie-Min-Seq: <seq>` or
`?minSeq=<seq>` on canonical durable reads (including board/thread/post, mail,
direct-message, and notification reads) and lag-aware derived views. If the
local read store or named view's `appliedSeq` is lower than the requested
sequence, the server returns HTTP `425` with retryable
`projection_stale`, freshness `meta`, `minSeq`, `Retry-After`, and
`X-Budgie-Read-Your-Writes: stale`; fresh reads return the normal body with
`X-Budgie-Read-Your-Writes: satisfied`. The header form takes precedence when
both header and query are present. Gateways should derive `minSeq` from the
latest durable event sequence observed by the author. Browser clients should
likewise remember the latest durable result `seq` they observe from
command-envelope acks, command-log status reads, resolved HTTP ack envelopes
from REST aliases, or bare REST `AckResult` responses, then send it on
lag-aware reads when the user expects their own write to be visible. Clients may
retry retryable
`projection_stale` responses after the advertised retry delay before falling
back to a fresher region or surfacing the stale-read error.

Board summary reads use `summaries.boards` and may be
served from a worker-owned `board_summary_stats` materialized table when
available, with favorites, zaps, read markers, policy flags, and online
presence still checked at read time. Unread-thread summary reads use
`summaries.unread_threads` and may be served from a worker-owned
`unread_thread_summary_stats` materialized candidate table when available, with
board visibility, favorite folders, zaps, read markers, unread post counts, and
first-unread navigation still checked at read time. Clients must treat these
views as convergent summaries, not as a substitute for per-partition event
ordering.
...
Board rankings use the `rankings.boards` view and may be served from a
worker-owned `board_ranking_stats` materialized table when available, falling
back to the compatibility SQL aggregation until the table is populated.
Thread rankings use the `rankings.threads` view and may be served from a
worker-owned `thread_ranking_stats` materialized table when available; because
reaction writes are unordered side-store updates, that processor may refresh
without a new durable event while keeping the durable watermark at the current
head.
Reply rankings use the `rankings.replies` view and may be served from a
worker-owned `reply_ranking_posts` materialized table when available, with
current post body, redaction state, and board visibility still checked at read
time.
User rankings use the `rankings.users` view and may be served from a
worker-owned `user_ranking_stats` materialized table when available; because
login, online-time, trust, and reaction-received counters can change outside the
durable ordered event path, that processor may refresh without a new durable
event while keeping the durable watermark at the current head.
Blessing rankings use the `rankings.blessings` view and may be served from a
worker-owned `blessing_ranking_stats` materialized table when available.
Archive rankings use the `rankings.archives` view and may be served from a
worker-owned `archive_ranking_stats` materialized table when available, with
current board visibility still checked at read time.
...
| `GET /api/v1/commands/{id}?commandPartitionKind=&commandPartitionKey=&commandOffset=` | inspect an authoritative command-log receipt |
...
attachPost     { id?, post*, filename*, contentType, sizeBytes, stagedBlobId? } -> post.attachment_added
...
- Binary uploads use `POST /api/v1/posts/{id}/attachments` with multipart
  ...
  Authoritative command-log submitters stage bytes in shared blob staging and
  include `stagedBlobId` in the queued command; the writer promotes those bytes
  atomically with the attachment event. If the writer cannot see the staged
  bytes it reports retryable `blob_staging_required`; regional clients without
  shared staging should route this path to the write region.
...
- `postCommitBody`, when present, is the text used by projection workers for
  post-created side effects such as mentions and reply notifications. Quoted
  replies use it to exclude quoted source text while keeping `body` as the
  displayable post content.
...
votePoll        { poll*, option* }                             -> ack + live poll.voted
...
setBoardSettings { board*, anonymousAllowed?, readOnly?, noReply?,
                   ...
                   statsExcluded?, zapAllowed? }             -> board.settings_set
setBoardMemberRequirements { board*, minLoginCount?, minPostCount?,
                   ...
                   approvalMode? }                              -> board.member_requirements_set
setRecommendedBoard { board*, recommended*, note?, position? } -> board.recommended_set
setBoardModerator { board*, user*, moderator*, position? }   -> board.moderator_set
setBoardMember { board*, user*, member*, title?,
                 ...
                 canSetBoardSettings? }                      -> board.member_set
applyBoardMembership { board*, note }                         -> board.member_application_submitted
                                                               (+ board.member_application_reviewed when auto-approved; ack id=application)
reviewBoardMembership { application*, status*, title, note }   -> board.member_application_reviewed
leaveBoardMembership { board* }                               -> board.member_set
curatePost     { post*, kind, title, path, note }             -> digest.entry_upserted
curateThread   { thread*, kind, title, path, note }           -> digest.entry_upserted
removeDigestEntry { entry* }                                  -> digest.entry_removed
updateDigestEntry { entry*, title?, path?, note? }             -> digest.entry_updated
setDigestEntryBody { entry*, body, reset? }                    -> digest.entry_body_set
createDigestDirectory { board*, kind?, path* }                 -> digest.directory_set
moveDigestPath { board*, kind?, fromPath*, toPath* }            -> digest.path_moved
copyDigestPath { board*, kind?, fromPath*, toPath* }            -> digest.path_copied
deleteDigestPath { board*, kind?, path* }                       -> digest.path_deleted
...
- Unscoped `GET /api/v1/search` applies the same member-read board filter as
  thread and post reads; scoped searches still reject unreadable boards. In
  external-index deployments, the search backend returns candidate post IDs
  only; the server hydrates those IDs from projections and applies current
  redaction, board-scope, and member-read policy before returning results.
...
- `GET /api/v1/boards/resident-feed` returns latest non-redacted posts from
  ...
  KBS-style resident-board aggregate reading. The response is served from the
  `feeds.resident` materialized view when available, falls back to the
  compatibility SQL projection otherwise, and returns projection metadata for
  `feeds.resident`.
- `GET /api/v1/feed/latest` returns latest non-redacted posts across boards the
  caller can currently read, excluding generated/stat-excluded boards and
  caller-zapped boards. It supplies board/thread context for a global latest
  feed. The response is served from the `feeds.latest` materialized view when
  available, falls back to the compatibility SQL projection otherwise, and
  returns projection metadata for `feeds.latest`.
...
- Board membership requirements expose KBS-style admission knobs. `maxMembers`,
  ...
  marks are reaction counts received on authored posts and read through the
  configured counter store. `approvalMode` is `"manual"` or `"auto"`; auto mode
  immediately approves an eligible application.
...
setMailGroup   { group?, name*, members[] }                    -> mail.group_set
deleteMailGroup { group* }                                     -> ack only
attachMail     { id?, mail*, filename*, contentType?, sizeBytes?, stagedBlobId? } -> mail.attachment_added
updateMail     { mail*, mailbox?, read?, kept? }               -> mail.copy_updated
deleteMail     { mail* }                                       -> mail.copy_updated
deleteMailRange { mail[] }                                     -> mail.copy_updated
sendDirectMessage { to*, body* }                               -> direct_message.sent
setDirectMessageSettings { policy* }                           -> direct_message.settings_set
markDirectMessageRead { message* }                             -> direct_message.read
deleteDirectMessage { message* }                               -> direct_message.deleted
```
...
- `attachments` on `sendMail` stores URL/metadata attachments. Binary mail file
  ...
  `GET /api/v1/mail/attachments/{id}`. Authoritative command-log submitters
  stage bytes in shared blob staging and include `stagedBlobId` in the queued
  command; the writer promotes those bytes atomically with the mail attachment
  event. If the writer cannot see the staged bytes it reports retryable
  `blob_staging_required`; regional clients without shared staging should route
  this path to the write region.
...
```
setUserRelationship { user*, kind*, active*, note }           -> user.relationship_set
setLoginWatch       { user*, active* }                        -> user.relationship_set[, notification.created]
...
```
setBoardFavorite { board*, favorite*, folderId?, position? }  -> board.favorite_set
setBoardZap      { board*, zapped* }                         -> board.zap_set
createFavoriteFolder { name*, parentId?, position? }          -> favorite_folder.created
updateFavoriteFolder { folder*, name?, parentId?, position? } -> favorite_folder.updated
deleteFavoriteFolder { folder* }                              -> favorite_folder.deleted
moveBoardFavorite { board*, folderId?, position? }            -> board.favorite_set
...
- `GET /api/v1/users/me/login-acl` returns the caller's login-host allow-list,
  ...
  including private LAN ranges. When enabled, password login fails unless the request
...
- `DELETE /api/v1/admin/users/{name}` hard-deletes an account row and its
  ...
  last-admin deletion are rejected. Reaction and poll-vote identity for the
  deleted user is removed through the configured counter store so unordered
  aggregate counts and received-reaction counters converge with the account
  purge.
...
```
- `sendChatLine` and `setPresence` are high-volume unordered commands. They
  bypass the authoritative/shadow command log and return immediate acks; their
  live events remain ephemeral.
- Chat recent-history reads are bounded side-store state rather than durable
  ordered-log replay. The active chat store may be SQL or a non-SQL KV-backed
  store, but it retains only bounded recent history. Clients should treat live
  `chat.line` events and `/chat/{room}/recent` as a best-effort room
  transcript, not as a source of durable board/thread ordering.
- Chat room rosters and room online counts are presence-store views keyed by
  active non-typing chat presence. Clients should treat `/chat/{room}/online`
  and room list online counts as live presence state, separate from transcript
  durability.
- `status`: `"active"` | `"idle"` | `"typing"` | `"offline"` |
  ...
  `mode`/`board`/`thread` when possible. The latest non-typing presence is
  persisted for online-user reads while `presence.update` remains an ephemeral
  live event. `"typing"` is a broker-only live hint: it does not overwrite the
  persisted online roster, satisfy login watches, or accrue online-time stats.
  Rapid identical online pings may be coalesced before writing the roster row;
  status changes, location changes, hidden/offline transitions, and periodic
...
board.settings_set { board, anonymousAllowed, readOnly, noReply, attachmentsAllowed, mailInAllowed, relayEnabled, memberReadMode, memberPostMode, statsExcluded, zapAllowed, by, ts }
board.member_requirements_set { board, minLoginCount, minPostCount, minTrustLevel, minScore, minBoardPostCount, minBoardOriginalPostCount, minBoardDigestCount, minBoardMarkCount, maxMembers, approvalMode, by, ts }
board.moderator_set { board, user, moderator, position, by, ts }
board.member_set { board, user, member, title, position, canManageMembers, canCurate, canModeratePosts, canModerateThreads, canAnnounce, canManagePolls, canSetBoardSettings, by, ts }
board.member_application_submitted { id, board, user, note, ts }
board.member_application_reviewed { application, board, user, status, title, reviewer, reviewNote, ts }
thread.new      { id, board, author, title, ts }
post.appended   { id, thread, author, body, rawBody, postCommitBody, signature, contentType, replyTo, tex, mailBack, sourcePost, sourceThread, sourceBoard, sourceAuthor, sourceAuthorId, sourceTitle, ts }
...
mail.group_set { id, ownerId, name, memberIds, ts }
mail.copy_updated { mail, userId, mailbox, read, kept, ts }
direct_message.sent { id, conversationId, fromUserId, from, toUserId, to, body, ts }
direct_message.read { messageId, userId, readAt, ts }
direct_message.deleted { messageId, userId, senderDeleted, recipientDeleted, ts }
direct_message.settings_set { userId, policy, ts }
user.relationship_set { userId, targetUserId, kind, active, note, ts }
notification.created { id, userId, kind, threadId, postId, actor, ts }
board.recommended_set { board, recommended, note, position, curatedBy, ts }
board.favorite_set { userId, board, favorite, folderId, position, ts }
favorite_folder.created { id, userId, parentId, name, position, ts }
favorite_folder.updated { id, userId, parentId, name, position, ts }
favorite_folder.deleted { id, userId, parentId, ts }
board.zap_set { userId, board, zapped, ts }
digest.entry_upserted { id, board, targetKind, targetId, kind, title, path, note, createdBy, ts }
digest.entry_updated { id, board, targetKind, targetId, kind, title, path, note, by, ts }
digest.entry_body_set { id, board, kind, body, edited, by, ts }
digest.entry_removed { id, board, kind, by, ts }
digest.directory_set { id, board, kind, path, createdBy, ts }
digest.path_moved { board, kind, fromPath, toPath, count, by, ts }
digest.path_copied { board, kind, fromPath, toPath, entryIds, directoryIds, count, createdBy, ts }
digest.path_deleted { board, kind, path, count, by, ts }
counter.checkpointed { complete, sourceHeadSeq, postReactions[{postId,count}], pollOptionVotes[{pollId,optionId,count}], ts }
```
...
post.reacted    { postId, thread, user, emoji, reactionCount, ts }
post.unreacted  { postId, thread, user, emoji, reactionCount, ts }
poll.voted      { poll, option, user, ts }
user.joined     { user, ts }
user.left       { user, ts }
```

Reaction and poll-vote commands mutate unordered side-store state. Their live
events are delivery hints for connected clients; projection reads are the source
for converged counts, and durable replay does not include every individual
reaction or vote.
...
- **Capabilities** (`welcome.capabilities`) let a client discover optional features (e.g. `ansi-art`, `sse`, `partition-cursors`) rather than assuming them. New optional events/commands are additive; clients ignore event types they don't recognize (forward-compatible).
