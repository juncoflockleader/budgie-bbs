...
- New accounts automatically receive the default `general` board favorite when
  that board exists.
- A `setBoardFavorite` command payload and durable `board.favorite_set` event.
- Folder commands:
  - `createFavoriteFolder` with durable `favorite_folder.created` events
  - `updateFavoriteFolder` with durable `favorite_folder.updated` events
  - `deleteFavoriteFolder` with durable `favorite_folder.deleted` events
  - `moveBoardFavorite` with durable `board.favorite_set` events
...
- Stat snapshots also create deterministic daily KBS `uonlinelog` /
  `listonline`-style `BBSLists` online-user roster threads showing visible
  online user sessions, distinct visible users, anonymous guests, current mode,
  public board/thread location, idle time, last-seen time, and from-host while
  masking member-read, generated, and `statsExcluded` board locations.
- Stat snapshots also create deterministic daily KBS `statBM` / lazy-board-
  manager-style `BBSLists` board-moderator activity threads showing public board
...
- Stat snapshots also create deterministic daily KBS `usage` / `board.xml`
  style `BBSLists` board-popularity list threads showing ranked public boards,
  article/thread totals, current board-online counts, moderators, and last
  activity times.
...
- Public recommended-article curation now also feeds a deterministic daily KBS
  `commend.xml`-style `BBSLists` recommended-article list thread. The list is
  built from public recommended digest entries and carries board, source kind,
  author, path, curator note, curator, updated time, and excerpt metadata while
  excluding member-read, hidden/staff-directory, generated system, and
  `statsExcluded` boards.
...
- Posting enforcement:
  ...
  - `statsExcluded` keeps otherwise readable/postable boards out of public
    ...
    online-user roster posts mask `statsExcluded` board locations, and
    board-moderator activity posts exclude `statsExcluded` boards.
...
- Admin-triggered and automatic stat snapshots lazily create the `BBSLists`
  ...
  including the KBS `statguy`-style user-activity rankings,
  `uonlinelog`-style online-user roster, `statBM`-style board-moderator
  activity report, `rcmdbrd`-style recommended-board list, and `commend`-style
...
- Board post redactions are projected into KBS-style board deletion bins.
  ...
  with `GET /api/v1/boards/{board}/deleted?kind=recycle|junk`, restoring a
  post removes it from the bin, board post moderators can range-delete and
  range-restore selected board posts, and they can clear selected or all board
  junk entries. Delegated board-member `canModeratePosts` permissions act as
  the board-local recycle/junk access list.
...
- User-scoped durable `user.relationship_set` events for friend/ignore rows.
