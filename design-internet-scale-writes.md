   ...
   (WS / SSE / SSH)  ───▶│   holds connections, owns subscriptions│
                       ...
                       owns part. │                       │ publish
                        ...
                        append   │
                                 ▼
                         ┌───────────────┐     ┌──────────────────────────┐
                         │ EVENTS log    │────▶│ REDIS HOT LAYER           │
                         │ (partitioned, │     │ coalesce view updates,    │
                         │  authoritative│     │ serve hot read slices     │
                         │  ordered)     │     └───────────┬──────────────┘
                         └───────────────┘                 │ flush/replay
                                                           ▼
                                                  ┌──────────────────────────┐
                                                  │ PROJECTION CONSUMERS      │
                                                  │ Postgres, search, feeds,  │
                                                  │ counters, rebuild jobs    │
                                                  └──────────────────────────┘
```

Three core substrates, plus one recoverable speed layer, are **deliberately
separated** (the current design conflates most of this in Postgres):
...
- **Hot/batching speed layer** = Redis. It is a volatile or soft-durable layer
  for coalescing derived-view writes and serving hot read slices. It is never
  the authoritative command or event log.
...
- **Presence / typing / live chat delivery** → broker-only, best-effort, never
  durable. Optional bounded chat transcripts live in a side store, not the
  ordered log. This keeps high-frequency ephemeral churn out of the durable log
  entirely.
...
## Redis batching and hot-serving layer

Redis is the speed layer between the durable event log and the slower query
stores, and beside the API for the hottest read paths. It can absorb bursts,
coalesce repeated updates for the same board/thread/user/feed, and serve recent
view slices without forcing every event to become an immediate Postgres write.

Good Redis responsibilities:

- **Command partition index:** cache logical command tails and committed
  offsets so writer assignment, lag checks, and promotion gates avoid repeated
  broker-wide scans. The durable command log remains authoritative and can
  rebuild the index after Redis loss.
- **Projection batch buffer:** event consumers write compact work items keyed by
  view and aggregate, then flush idempotent batches to Postgres, search, or
  materialized feed stores on a bounded interval. A one-minute freshness budget
  is acceptable for these derived views when the durable event log can replay the
  missed window.
- **Hot read cache:** store recent thread slices, board summaries, feed
  candidates, command status, presence-ish counters, and ranking windows in
  hashes, sorted sets, or streams with TTLs. API nodes fall back to Postgres or a
  rebuild/replay path on miss.
- **Write coalescing for unordered state:** high-frequency counters and presence
  hints can use Redis atomic operations or streams before checkpointing to the
  durable/rebuildable stores.

Hard guardrails:

- Redis loss must never lose authoritative posts, commands, or event offsets.
  The Kafka/Redpanda/NATS event log remains the source of truth.
- Redis can lose cache warmth or at most a bounded amount of unflushed
  *derived-view freshness*. Recovery is replay/backfill from the event log.
- Redis work items are idempotent by event id or `(partition, offset)`, and
  flush watermarks are stored durably outside Redis.
- Queues are bounded and backpressured. If Redis falls behind, clients see stale
  view metadata or fallback reads rather than silent correctness loss.
...
| Hot view batching/serving | **Redis Cluster** (Streams, sorted sets, hashes, TTLs) | DragonflyDB, KeyDB |
| Hot counters (reactions) | **ScyllaDB** counters / sharded Redis | Cassandra, Redis CRDTs |
| Queryable projections | **Postgres** read replicas (per region), fed in batches | CockroachDB, Scylla for hot views |
...
4. **Make global views asynchronous** (feeds, rankings, search, stats as stream
   jobs).
5. **Add Redis as the hot speed layer** for derived-view batching and hot read
...
## Appendix A: Phase 1 implementation — split delivery from durability via NATS

Phase 1 keeps Postgres as the authoritative log and replaces the live cross-node
wakeup when `budgied -nats <url>` is enabled. Without `-nats`, `appendEvent`
still does `bus.Publish` (local) **+ `pg_notify`** (metadata only), and every
node's `pq.Listener` fetches the event by `seq` before re-publishing locally.
With `-nats`, `NATSBus` sends the **full event body over NATS** on
scope-specific subjects, so receivers deliver from memory — no fetch-by-seq, no
DB read on the live delivery path. Each node subscribes upstream only to the
union of its local client scopes; multi-scope duplicates are deduped before the
event enters local gateway fanout. Durability, the advisory write lock,
projections, cursor-based gap replay, and the client protocol are all untouched.

```go
// internal/core/nats_bus.go
...
func (b *NATSBus) Publish(evt *proto.Event) {
	b.PublishLocal(evt) // local subscribers on this node
	if b.conn == nil {
		return
	}
	...
	for _, subject := range natsSubjectsForScopes(evt.Scopes) {
		_ = b.conn.Publish(context.Background(), subject, data)
	}
}
...
func (b *NATSBus) onRemote(data []byte) {
	...
	if !b.rememberRemote(remoteEventDedupeKey(env, w)) {
		return // same multi-scope event already reached this gateway
	}
	payload, err := unmarshalPayload(w.Event, w.Payload)
	if err != nil {
		return
	}
...
(`crossNodeViaBus` set when a `NATSBus` is injected via `WithBusFactory`):
...
The concrete `nats.go` adapter (implementing `NATSConn`) lives in
`internal/natsconn`. `cmd/budgied` has a `-nats <url>` flag (also
`BUDGIE_NATS_URL`) that dials it, builds `core.NewNATSBus(conn, nodeID)`,
injects it with `core.WithBusFactory`, and lets `Core.Run` call `Start()` to
begin consuming.
...
This removes the read amplification and cleanly separates delivery from
durability, which every later phase builds on.
